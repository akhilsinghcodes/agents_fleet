...
Browser (HeadroomChat.tsx)
        ...
        v
Agents Fleet server  (apps/server/src/routes/litellm.ts)
        |
...
**LiteLLM returns 401 through the proxy**
  The proxy forwards the Authorization header from Agents Fleet. Confirm
...
| LITELLM_BASE_URL      | (unset)           | Agents Fleet server reads this for non-headroom sessions |
| LITELLM_API_KEY       | (unset)           | Agents Fleet server reads this for all LiteLLM calls     |

The proxy URL seen by the Agents Fleet server is stored per-session in SQLite as
...
Agents Fleet-level knob to tune this. If you need a different threshold, refer
