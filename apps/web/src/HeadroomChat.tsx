...
const HEADROOM_PROXY_URL = "http://localhost:8787/v1";
const HEADROOM_PROXY_BASE = "http://localhost:8787";

type Props =
  | { mode: "new"; onCreated: (session: Session) => void }
...
export default function HeadroomChat(props: Props) {
  ...
  const canSend = useMemo(
    () => !busy && input.trim().length > 0 && proxyOnline === true,
    [busy, input, proxyOnline],
  );

  const wsRef = useRef<WebSocket | null>(null);
  const subscribedSessionRef = useRef<string | null>(null);
  const startPromiseRef = useRef<Promise<Session> | null>(null);
  const activeStreamRef = useRef<{ sessionId: string; assistantItemId: string } | null>(null);
  ...
  useEffect(() => {
    getHeadroomStats(HEADROOM_PROXY_BASE)
      .then((res) => setProxyOnline(res.configured))
  ...
  async function subscribeChatWs(ws: WebSocket, sessionId: string) {
    ...
    if (ws.readyState !== WebSocket.OPEN) {
      await new Promise<void>((resolve, reject) => {
        ...
        const onErr = () => { ws.removeEventListener("open", onOpen); reject(new Error("WebSocket error")); };
  ...
  async function startIfNeeded() {
    ...
    if (props.mode !== "new") throw new Error("Internal error: startIfNeeded called for existing session");
    if (repoPath.trim().length === 0) throw new Error("repoPath is required");
    if (startPromiseRef.current) return startPromiseRef.current;

    startPromiseRef.current = createLiteLlmSession({
      repoPath: repoPath.trim(),
      model,
      budgetUsd: budgetUsd.trim().length > 0 ? Number(budgetUsd) : undefined,
      headroomBaseUrl: HEADROOM_PROXY_URL,
    }).then((created) => {
      setSession(created);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const json = await getSessionArtifacts({ sessionId: props.sessionId, limit: 2000 });
        if (cancelled) return;

        const configArtifact = json.artifacts.find((a) => a.kind === "litellm_chat_config_v1");
        if (configArtifact) {
        ...
        setBudgetUsd(typeof current.budget_usd === "number" ? String(current.budget_usd) : "");

        const nextItems: Item[] = json.artifacts
          .filter((a) => a.kind === "litellm_chat_user_message_v1" || a.kind === "litellm_chat_assistant_message_v1")
          .map((a) => {
            const parsed = JSON.parse(a.content) as { text?: unknown };
            return {
              kind: "message",
              id: a.id,
              role: a.kind === "litellm_chat_user_message_v1" ? ("user" as const) : ("assistant" as const),
  ...
  async function send() {
    ...
    try {
      sessionId = props.mode === "existing" ? props.sessionId : (await startIfNeeded()).id;
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setBusy(false);
      setInput(text);
    ...
    <div style={{ display: "grid", gridTemplateRows: "auto 1fr auto", gap: 10 }}>
      <div style={{ display: "grid", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 16 }}>Headroom</div>
            <div style={{ fontSize: 12, color: "#6b7280", marginTop: 4 }}>
              LiteLLM chat with context compression via headroom proxy.
            </div>
          </div>
          {proxyOnline === false && (
            <div style={{
              ...
              Headroom proxy not running. Start with: <code>pnpm dev:one</code>
            </div>
          )}
          {proxyOnline === true && (
            <div style={{
              marginLeft: "auto", fontSize: 12, color: "#065f46",
              background: "#d1fae5", border: "1px solid #6ee7b7",
              borderRadius: 8, padding: "6px 12px",
            }}>
              Proxy connected at {HEADROOM_PROXY_BASE}
            </div>
          )}
        </div>
        ...
        {session ? (
          <div style={{
            ...
            {session.status === "running" && (
              <button
                onClick={() => {
                  stopSession(session.id).then((updated) => setSession(updated)).catch((e) => setError(String(e)));
                }}
                style={{ padding: "3px 12px", borderRadius: 6, border: "1px solid #dc2626", background: "#fef2f2", color: "#dc2626", cursor: "pointer", fontSize: 12, fontWeight: 500, flexShrink: 0 }}
              >
                Stop
              </button>
            )}
      ...
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, overflow: "auto", background: "#0b0f14", color: "#d6dde6" }}>
        {items.length === 0 ? (
          <div style={{ fontSize: 12, color: "#9ca3af" }}>Send a message to start.</div>
        ) : null}

        {items.map((item) => {
          if (item.kind === "tool") {
            return (
              <div key={item.id} style={{ marginBottom: 12, border: "1px solid #374151", borderRadius: 8, padding: 8, background: "#111827" }}>
                <div style={{ fontSize: 11, color: "#9ca3af", marginBottom: 4 }}>
                  tool: bash • {new Date(item.ts).toLocaleTimeString()}
                </div>
                <div style={{ fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', fontSize: 12, color: "#fbbf24", marginBottom: 6, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>
                  $ {item.command}
                </div>
                {item.status === "pending" && (
                  <div style={{ display: "flex", gap: 6 }}>
                    <button
                      onClick={() => {
                        const ws = wsRef.current; const sid = subscribedSessionRef.current;
                        if (!ws || !sid) return;
                        setItems((prev) => prev.map((i) => i.kind === "tool" && i.toolCallId === item.toolCallId ? { ...i, status: "approved" } : i));
                    ...
                    <button
                      onClick={() => {
                        const ws = wsRef.current; const sid = subscribedSessionRef.current;
                        if (!ws || !sid) return;
                        setItems((prev) => prev.map((i) => i.kind === "tool" && i.toolCallId === item.toolCallId ? { ...i, status: "denied" } : i));
                ...
                {item.status === "approved" && !item.stdout && !item.stderr && <div style={{ fontSize: 11, color: "#6b7280" }}>Running...</div>}
                {item.status === "denied" && <div style={{ fontSize: 11, color: "#f87171" }}>Denied</div>}
                {item.status === "done" && (
                  <div>
                    {item.stdout && <pre style={{ margin: 0, fontSize: 11, color: "#d1fae5", whiteSpace: "pre-wrap", wordBreak: "break-all" }}>{item.stdout}</pre>}
                    {item.stderr && <pre style={{ margin: 0, fontSize: 11, color: "#fca5a5", whiteSpace: "pre-wrap", wordBreak: "break-all" }}>{item.stderr}</pre>}
                    <div style={{ fontSize: 10, color: "#6b7280", marginTop: 4 }}>exit {item.exitCode} • {item.durationMs}ms{item.truncated ? " • truncated" : ""}</div>
                  </div>
                )}
              </div>
            );
          ...
          return (
            <div key={item.id} style={{ marginBottom: 12 }}>
              ...
              <div style={{ whiteSpace: "pre-wrap", fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', fontSize: 12 }}>
                {item.text}
              </div>
            </div>
          );
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
        <textarea
          ...
          onFocus={() => void ensureSessionReady()}
          onMouseDown={() => void ensureSessionReady()}
          placeholder={proxyOnline === false ? "Headroom proxy not running..." : "Write a message..."}
          disabled={proxyOnline === false}
          style={{ flex: 1, padding: "10px 12px", borderRadius: 12, border: "1px solid #d1d5db", resize: "none", overflow: "hidden", fontFamily: "inherit", fontSize: "inherit", lineHeight: 1.5 }}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void send(); } }}
        />
        <button
          onClick={() => void send()}
          disabled={!canSend}
          style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid #111827", background: canSend ? "#111827" : "#e5e7eb", color: canSend ? "white" : "#6b7280", cursor: canSend ? "pointer" : "not-allowed" }}
        >
          {busy ? "Sending..." : "Send"}
        </button>
      </div>
